  ...
  async function handleCadastrar() {
    ...
    try {
      ...
      const detailsRes = await fetch(
        `/api/tmdb/details?type=movie&tmdbId=${item.tmdbId}`
      );
      ...
      const res = await fetch("/api/requests", {
        ...
        body: JSON.stringify({ requestedTitle: item.title, type: "MOVIE", tmdbId: item.tmdbId, posterUrl: item.posterPath }),
      });
      ...
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <div className="flex gap-3 p-5 pt-0">
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white font-medium transition text-sm disabled:opacity-50"
          >
  ...
  return (
    <div className="mb-10">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
          {items.map((item) => (
            <MovieCard key={item.tmdbId} item={item} onClick={onCardClick} />
          ))}
        </div>
      )}
    </div>
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-8">
        ...
        <p className="text-zinc-400 text-sm mt-1">
          Clique em qualquer card para cadastrar o filme no sistema.
        </p>
      </div>

      {/* Seções */}
      <Section
        title="🎥 Em Cartaz Hoje"
        items={nowPlaying}
        loading={loadingNow}
