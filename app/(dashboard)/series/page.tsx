    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <div className="flex gap-3 p-5 pt-0">
          ...
          <button onClick={handleCadastrar} disabled={loading} className="flex-1 px-4 py-2 rounded-lg bg-purple-600 hover:bg-purple-500 text-white font-medium transition text-sm disabled:opacity-50">{loading ? "Cadastrando..." : "Cadastrar"}</button>
        </div>
      </div>
    </div>
  );
    ...
    <button onClick={() => onClick(item)} className="group relative bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden hover:border-zinc-600 hover:shadow-lg transition-all duration-200 text-left">
      <div className="aspect-[2/3] bg-zinc-800 relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
          <span className="bg-purple-600 text-white text-xs font-semibold px-3 py-1.5 rounded-full">+ Cadastrar</span>
        </div>
      </div>
      <div className="p-2">
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        ...
        <p className="text-zinc-400 text-sm mt-1">Clique em qualquer card para cadastrar a série no sistema.</p>
      </div>
