...
import { Film, Tv, Download, CheckCircle, ClipboardList, Server } from 'lucide-react'
...
export default async function DashboardPage() {
  ...
  const cards = [
    { label: 'Total de Títulos', value: stats.totalTitles, icon: Film, color: 'text-blue-400', bg: 'bg-blue-400/10 border-blue-400/20' },
    { label: 'Aguardando Download', value: stats.pendingTitles, icon: Download, color: 'text-yellow-400', bg: 'bg-yellow-400/10 border-yellow-400/20' },
    { label: 'Pedidos Abertos', value: stats.openRequests, icon: ClipboardList, color: 'text-red-400', bg: 'bg-red-400/10 border-red-400/20' },
    { label: 'Disponível em P1', value: stats.p1Count, icon: Server, color: 'text-green-400', bg: 'bg-green-400/10 border-green-400/20' },
    { label: 'Disponível em P2', value: stats.p2Count, icon: Server, color: 'text-primary', bg: 'bg-primary/10 border-primary/20' },
    { label: 'Filmes', value: stats.movies, icon: Film, color: 'text-purple-400', bg: 'bg-purple-400/10 border-purple-400/20' },
  ]
  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {cards.map((card) => (
          <div key={card.label} className={`bg-card border rounded-xl p-6 ${card.bg}`}>
      ...
      <div className="mt-8 grid grid-cols-2 gap-4">
        <div className="bg-card border border-border rounded-xl p-6">
          <h3 className="font-semibold mb-4 flex items-center gap-2"><Film className="w-4 h-4 text-primary" /> Por Tipo</h3>
        ...
        <div className="bg-card border border-border rounded-xl p-6">
          <h3 className="font-semibold mb-4 flex items-center gap-2"><Server className="w-4 h-4 text-primary" /> Servidores</h3>
          <div className="space-y-3">
            <div className="flex items-center justify-between">
